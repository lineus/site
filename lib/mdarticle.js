...
MDArticle.prototype.generateFile = function() {
  this.checkParsed();
  let ret = Object.create(null);
  for (let prop in this.file) {
    ret[prop] = this.file[prop];
  }
  ret.published = new Date(this.data.published).toDateString();
  ret.rssPublished = new Date(this.data.published).toUTCString();
  ret.rssSynopsis = this.data.synopsis;
  ret.tags = this.genSearch().level[1].tokens;
  for (let prop in this.data) {
    if (prop === 'published') {
      continue;
    }
    ret[prop] = this.data[prop];
  }
  return ret;
};
...
MDArticle.prototype.markUpdated = function() {
  ...
  if (this.data.status !== 'UPD' || this.data.published < 1) {
    throw new Error(`ArticleNotUpdatable: ${this.path}`);
  }
